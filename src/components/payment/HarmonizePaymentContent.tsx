...
import { Checkbox, Input, Modal, Spin, type CheckboxProps } from 'antd';
...
export default function HarmonizePaymentContent(
  ...
  //   let charge = (props.data.settings.percentage * props.data.TotalAmount / 100);
  ...
  const toggleWalletModal = () => {
    ...
    if (String(amountToPay) > String(props.data.TotalAmount).replaceAll(",", "")) {
  ...
  function completePayment() {
    if (userOTPValue.length === 4) {
      harmonizeWalletNotification({
        ...
        amountPaid: amountToPay ? amountToPay : props.data.TotalAmount,
        pin: userOTPValue
      })
    } else {
    ...
    //         router.push(
    //           `/payment/harmonize/generate-receipt?reference=${data.tx_ref}&tx_id=${data.transaction_id}&payment_method=bank`
    //         );
    //       } else {
    //         showSnackBar({
    //           severity: "error",
  ...
  useEffect(() => {
    ...
    let amountToUse = String(amount).replaceAll(",", "");
  ...
  const updateAmountToPay = (e: any) => {
    ...
    if (target <= String(props.data.TotalAmount).replaceAll(",", "")) {
      if (charge < props.data.settings.min) {
        setServiceCharge(props.data.settings.min);
      } else if (charge > props.data.settings.max) {
        setServiceCharge(props.data.settings.max);
        ...
        displayPaymentContent ?
          <div>
            ...
            <form onSubmit={onSubmit} className="flex flex-col gap-10">
              {props.data.bulkBill.Webguid.map((item: any, index: number) => {
                return (
                  <div
                    key={item.webguid}
                  ...
                  >
                    ...
                    {/* <div className="flex flex-col gap-2">
                      ...
                      <TextInput className="h-[4rem]" disabled value={item.AgencyCode} placeholder="" />
                    </div> */}
                    <div className="flex flex-col gap-2">
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      <TextInput className="h-[4rem]" disabled value={item.AgencyName} placeholder="" />
                    </div>
                    <div className="flex flex-col gap-2">
                      <p className="font-medium">Revenue Name</p>
                      <TextInput className="h-[4rem]" disabled value={item.RevName} placeholder="" />
                    </div>
                    {
                      index === 1 ?
                        <div className="flex flex-col gap-2">
                          <p className="font-medium">Total Amount*</p>
                          <TextInput className="h-[4rem]" disabled value={props.data.TotalAmount} placeholder="" />
                        </div> : ""}
                    <div className="flex flex-col gap-2">
                ...
                <TextInput className="h-[4rem]" disabled defaultValue={props.data.TotalAmount} />
              </div> */}
                  </div>
